...
const authenticateUser = async (data) => {
    const user = await db.getUser({ email: data.email });
    if (!user) {
...
const authController = {
    signup: async (req, res) => {
        ...
        const existingEmail = await db.getUser({
            $or: [{ email: data.email }, { username: data.username }],
        });
        if (existingEmail) {
            const err = new CustomError(409, "Already an account on this Email or Username");
            throw err;
        }
